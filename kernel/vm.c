...
// pagetable_t proc_kpt_init(struct proc *p) {
...
//   uvmmap(k_pgtbl, TRAPFRAME, (uint64)(p->trapframe), PGSIZE, PTE_R | PTE_W);
...
void uvmmap(pagetable_t page, uint64 va, uint64 pa, uint64 sz, int perm) {
...
void ukvmunmap(pagetable_t page, uint64 va, uint64 size) {
  uint64 a, last;
  pte_t* pte;
